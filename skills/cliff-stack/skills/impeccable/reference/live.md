...
1. `live.mjs`: boot. If the request names or implies a file, route, or app inside a monorepo, infer the concrete path and run `node "<SKILL_DIR>/scripts/live.mjs" --target <path>` instead; then run the rest of this live session from the returned `projectRoot`.
...
```
node "<SKILL_DIR>/scripts/live-poll.mjs" --stream   # stays running; one JSON line per event
...
```bash
node "<SKILL_DIR>/scripts/live-insert.mjs" --id EVENT_ID --count EVENT_COUNT --position after \
...
```bash
node "<SKILL_DIR>/scripts/live-poll.mjs" --reply EVENT_ID steer_done ["Optional short note for a browser toast"]
```

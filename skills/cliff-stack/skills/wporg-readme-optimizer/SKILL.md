...
- **Nom et slug : marques d'autrui (guideline 17)**. Une marque ou un nom de projet tiers ne peut être ni le seul ni le premier terme du slug sans preuve de représentation ; forme admise : « <Nom original> for <Marque> » (exemple officiel : « Dancing Sloths for Superbox »), le branding original étant recommandé. Plugin Check (`Trademarks_Check`, v2.1.0 relue) le contrôle sur le nom du header, le nom du readme et le slug via sa constante `TRADEMARK_SLUGS` : un terme finissant par `-` ne peut pas **commencer** le slug, un terme sans `-` ne peut apparaître **nulle part** ; seules exceptions « for-use » (`FOR_USE_EXCEPTIONS`) : motifs `for-`/`with-`/`using-`/`and-` + terme ; tout slug commençant par un préfixe de `PORTMANTEAUS` est signalé. Plugin Check émet ces cas en **warning**, mais c'est la guideline 17 que l'équipe de revue applique. Lire la liste dans le code de Plugin Check installé plutôt que de la recopier (elle évolue). Un titre optimisé pour la recherche qui viole ces règles est rejeté : la conformité passe avant le mot-clé.

...
Concevoir les séquences email d'un freemium plugin/SaaS. Complémentaire : la plateforme de facturation/licence du projet (billing, trials, **dunning**), [[copywriting-landing]] (le copy des emails de conversion), [[pricing-strategist]]/[[marketing-psychology]] (offre & persuasion).
...
2. **D'où vient l'email ?** ⚠️ **wordpress.org ne transmet AUCUN email** au téléchargement du plugin gratuit. Pour un freemium WP.org, la **seule source d'email à l'activation = l'opt-in fourni par le SDK de licence/télémétrie du plugin, s'il en a un** (écran de consentement à l'activation) ; sinon, un formulaire d'inscription intégré au plugin. C'est un opt-in **produit/transactionnel**, pas un opt-in marketing. Concevoir là la capture, et un **opt-in marketing dédié distinct** (case séparée, non pré-cochée) pour le démarchage. Sans cette étape, il n'y a pas de liste.
...
4. **Trial-to-paid** (si la plateforme de facturation propose un funnel trial activé) — séquence d'expiration.
...
Le **dunning** (retries + emails de carte expirée/échec de paiement) est géré **par la plateforme de facturation/licence du projet**. Ce skill couvre seulement la **coordination** :
...
- Écouter les **webhooks de la plateforme de facturation** comme triggers de changement de lifecycle stage — sans dupliquer la logique de retry.

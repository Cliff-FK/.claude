...
### Prestige / Even-Price Effect (l'inverse du charm)
Les prix ronds ($100, $500) signalent qualité et fluidité de traitement ; le haut de gamme et les restaurants arrondissent VOLONTAIREMENT pour l'image de marque. Le charm (.99 / .95) crie "promo / value" — contre-productif sur un produit premium.

**Marketing application**: Aligner la terminaison sur le positionnement. Charm pour un plan value / starter, prix rond pour Enterprise / premium. Ne jamais mettre .99 sur ce qui doit paraître exclusif.

### Mécanisme — pourquoi le charm marche (théories concurrentes)
Le 9-ending domine (~60 % des prix affichés ; ~28 % en 5-ending) mais pour des raisons débattues :
1. **Left-digit anchoring** (Thomas & Morwitz 2005) : on ancre sur le chiffre de gauche, $1.99 est perçu "proche de 1", pas de 2. Effet réel mesuré sur le choix (Manning & Sprott 2009).
2. **Prospect theory / point de référence** : $19.98 pose $20 comme référence ; le prix juste-en-dessous est vécu comme un "gain".
3. **Effet image vs effet niveau** (Stiving & Winer 1997, données scanner) : le 9 agit À LA FOIS comme signal de solde (image) ET par sous-estimation du montant (level) — deux canaux distincts, additifs.

### Price-Band Threshold (rester sous un seuil de filtre)
Quand l'acheteur filtre par tranches (immobilier, marketplace, filtre "< 50 €"), terminer à 49 999 plutôt que 50 000 garde l'item dans la tranche basse — il est vu par plus d'acheteurs. L'enjeu n'est pas la perception du chiffre mais l'ÉLIGIBILITÉ au filtre.

**Marketing application**: Repérer les seuils de filtre réels de ton canal de vente et caler le prix juste sous le seuil le plus consulté.

### Descending-Order Presentation
Présenter les prix du plus cher au moins cher (vs l'inverse) augmente le consentement à payer : le premier prix ancre haut. Cohérent avec l'**Anchoring Effect**.

### Sale-Ending Differentiation (convention opérationnelle)
Réserver une terminaison aux prix normaux et une autre aux soldes (ex. 95 = plein tarif, 50 = solde) : le client repère les promos d'un coup d'œil, et l'effet "image de solde" du 9 reste crédible parce qu'il n'est pas galvaudé.

### ⚠️ Quand le charm pricing NE marche pas (contre-indications)
- **Taxe ajoutée à la caisse** (US) ou **arrondi monétaire** (disparition du centime : Australie, certaines zones euro) : le .99 affiché n'est pas le prix payé → l'effet left-digit s'annule.
- **Cartes / sans-contact** : la friction du rendu de monnaie (origine historique anti-vol — "ouvrir le tiroir-caisse" ; *baťovská cena*, ~1920) a disparu.
- **Acheteurs sophistiqués / B2B** : données scanner modernes MIXTES ; même des enfants évaluent correctement le coût réel. L'effet est plus faible qu'on ne croit.
- **Régulation** : Israël (2013) a interdit les prix en .99 jugés trompeurs.
- **Seuil de digit, à l'inverse** : faire FRANCHIR un seuil de gauche ($4.99 → $5.00) maximise l'effet dissuasif (taxe cigarettes, MacKillop 2014) — utile quand tu veux que la hausse SE VOIE.

**Règle de décision** : charm = produits value, prix affiché = prix payé, marché sensible au prix. Rond = premium / B2B / image. Toujours A/B tester — l'effet est réel mais petit et contextuel, jamais une garantie.

...
- **Slug de preset → variable CSS : passer par `_wp_to_kebab_case`**, jamais par le slug brut. `WP_Theme_JSON` génère `--wp--preset--<type>--<kebab(slug)>` et les classes `has-<kebab(slug)>-*` : `3xl` → `3-xl`, `cardShadow` → `card-shadow`, `h1` → `h-1`. Une variable construite depuis le slug brut pointe vers une propriété inexistante et échoue en silence.
- **Aucune fonction de traduction avant `init`** : depuis 6.7, `_load_textdomain_just_in_time` émet un `_doing_it_wrong` si un domaine est chargé avant la fin de `after_setup_theme` (seulement si un fichier de traduction existe pour la locale : invisible en anglais, donc tester dans une locale traduite).

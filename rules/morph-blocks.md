...
- **Portée** : les chemins ci-dessus sont les fichiers du moteur relevés le 23/09/2026 (un fichier ajouté au moteur s'ajoute ici) ; la règle ne vaut que si le fichier appartient bien au moteur de variantes par écran (fonctions `wbd_rsp_*`, constantes `WBD_RSP_*`, globales JS `wbdRsp*`, clés `_morph_tablet` / `_morph_mobile`). Le moteur vit dans le thème depuis le 08/09/2026 ; l'ancienne extension `morph-blocks`, sa couche de licence et son build gratuit n'existent plus. **Selon la version du thème, le code porte `wbd_rsp_*` / `WBD_RSP_*` / `wbdRsp*` (wp-builder-2026 depuis le 24/09/2026) ou encore `morph_blocks_*` / `MORPH_BLOCKS_*` / `morphBlocks*` (copies antérieures) : chercher les deux avant de conclure à une absence.** Les noms de DONNÉES gardent `morph` dans toutes les versions (clés de contenu, table `morph_blocks_cache`, options, métas, cron, supports `morphBlocks*Attrs`).

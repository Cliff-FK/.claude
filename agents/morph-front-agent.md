...
- `includes/prepaint.php` — inline `<head>` snippet (`wp_head` prio 1): MutationObserver firing BEFORE first paint, synchronous swap via `template`/`replaceChild` on `data-morph-sig` at node insertion; sets `window.__morphBlocksInit`; `morph_blocks_prepaint_arm()` (skip if no cache) + `morph_blocks_prepaint_emit()`.
...
- **→ PREPAINT ↔ STORE phantom link (CONFIRMED, open)**: serve/prepaint claim `window.__morphBlocksInit` lets store.js skip its initial swap — but **store.js never reads it** (grep is zero: `prepaint.php` sets it, no consumer in `store.js`). Result: systematic double-swap on mobile/tablet (prepaint then store redoes everything — idempotent via morphdom but wasteful). Decide: implement the skip in store.js OR drop the claim from the contract. Do not "fix" by guessing.
- **→ CROSS-POST LEAK at prepaint (CONFIRMED, open)**: prepaint `readReg()` does `querySelectorAll('script[id^="morph-blocks-"]')` and merges ALL JSON blobs **without** the footer's `$sigs_seen` filter → on archives/multi-post pages, other posts' sigs merge in. The non-leak invariant serve enforces for the footer is NOT carried by prepaint. Any prepaint/registry change must preserve per-post isolation.

...
1. **Script à déposer par FTP (interrompu).** Objectif : `scripts/build-drop.php` génère un fichier PHP unique à poser à la racine de n'importe quel projet ; son URL demande un jeton (formulaire POST, cookie de session, expiration codée, suppression automatique), analyse par étapes, affiche les constats, propose uniquement les actions pertinentes (quarantaine, restauration d'un fichier du cœur, retrait chirurgical d'un bloc .htaccess, suppression de lignes de base, invalidation des mots de passe et sessions, régénération des clés), avec aperçu, sauvegarde, confirmation, journal et annulation ; bouton final « terminer et supprimer ».
   - État : seul `detect.php` a été modifié (+403 / -192 lignes, non commité) pour tenir les projets PHP non WordPress. Unit 21/21 repasse ; **mutations et acceptation à relancer** pour confirmer l'absence de régression. Le module d'actions (`scripts/drop/actions.php`), le build, l'interface, `references/drop-script.md` et `tests/drop-script.php` ne sont **pas écrits**.
   - Décider d'abord : garder ou annuler la modification de `detect.php` (`git diff` puis commit ou `git checkout` du fichier).
   - Tests prévus : `php -S` sur une copie, parcours HTTP réel (jeton, CSRF, expiration, actions fichiers avec effet disque, sauvegarde, annulation, suppression finale) ; actions base testées en live seulement si une base jetable locale est permise, sinon le dire.
   - Critique adversariale centrée sur la sécurité du fichier déposé (il tourne sur un site piraté : pas d'exécution de commande ni de lecture de fichier arbitraires, actions bornées aux constats).
...
3. **Sécurité** : régénérer la clé Freemius qui figurait en clair dans une mémoire supprimée (elle reste dans l'historique git de `~/.claude`).
